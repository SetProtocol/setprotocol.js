...
import { TakerWalletWrapper as ContractArtifacts } from 'set-protocol-contracts';
...
export class TakerWalletWrapperContract extends BaseContract {
  public addAuthorizedAddress = {
    async sendTransactionAsync(
      _authTarget: string,
      txData: TxData = {},
    ): Promise<string> {
      const self = this as TakerWalletWrapperContract;
  ...
  static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<TakerWalletWrapperContract> {
    const currentNetwork = web3.version.network;
    const { abi, networks }: { abi: any; networks: any } = ContractArtifacts;
    const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

    return new TakerWalletWrapperContract(web3ContractInstance, defaults);
  }
  static async at(
    address: string,
    web3: Web3,
    ...
    if (contractExists) {
      ...
      throw new Error(
        CONTRACT_WRAPPER_ERRORS.CONTRACT_NOT_FOUND_ON_NETWORK('TakerWalletWrapper', currentNetwork),
      );
    }
  }
