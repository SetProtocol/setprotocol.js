...
describe('SetTokenWrapper', () => {
  ...
  describe('getFactoryAsync, getComponentsAsync, getNaturalUnitAsync, getUnitsAsync', async () => {
    ...
    async function subject(): Promise<any> {
      const factoryAddress = await setTokenWrapper.getFactoryAsync(subjectSetTokenAddress);
      const components = await setTokenWrapper.getComponentsAsync(subjectSetTokenAddress);
      const naturalUnit = await setTokenWrapper.getNaturalUnitAsync(subjectSetTokenAddress);
      const units = await setTokenWrapper.getUnitsAsync(subjectSetTokenAddress);
