import Web3 from 'web3';
import Contract from "web3/eth/contract";
import { Tx } from "web3/eth/types";
import { BigNumber } from 'set-protocol-utils';
import { BaseContract } from '../base_contract';
export declare class HistoricalPriceFeedContract extends BaseContract {
    lastUpdatedAt: {
        callAsync(defaultBlock?: any): Promise<BigNumber>;
    };
    dataDescription: {
        callAsync(defaultBlock?: any): Promise<string>;
    };
    historicalPriceData: {
        callAsync(defaultBlock?: any): Promise<[BigNumber, BigNumber]>;
    };
    updateFrequency: {
        callAsync(defaultBlock?: any): Promise<BigNumber>;
    };
    renounceOwnership: {
        sendTransactionAsync(txData?: Tx): Promise<string>;
        estimateGasAsync(txData?: Tx): Promise<number>;
        getABIEncodedTransactionData(txData?: Tx): string;
        callAsync(txData?: Tx): Promise<void>;
    };
    owner: {
        callAsync(defaultBlock?: any): Promise<string>;
    };
    isOwner: {
        callAsync(defaultBlock?: any): Promise<boolean>;
    };
    medianizerInstance: {
        callAsync(defaultBlock?: any): Promise<string>;
    };
    transferOwnership: {
        sendTransactionAsync(newOwner: string, txData?: Tx): Promise<string>;
        estimateGasAsync(newOwner: string, txData?: Tx): Promise<number>;
        getABIEncodedTransactionData(newOwner: string, txData?: Tx): string;
        callAsync(newOwner: string, txData?: Tx): Promise<void>;
    };
    poke: {
        sendTransactionAsync(txData?: Tx): Promise<string>;
        estimateGasAsync(txData?: Tx): Promise<number>;
        getABIEncodedTransactionData(txData?: Tx): string;
        callAsync(txData?: Tx): Promise<void>;
    };
    read: {
        callAsync(_dataDays: BigNumber, defaultBlock?: any): Promise<BigNumber[]>;
    };
    changeMedianizer: {
        sendTransactionAsync(_newMedianizerAddress: string, txData?: Tx): Promise<string>;
        estimateGasAsync(_newMedianizerAddress: string, txData?: Tx): Promise<number>;
        getABIEncodedTransactionData(_newMedianizerAddress: string, txData?: Tx): string;
        callAsync(_newMedianizerAddress: string, txData?: Tx): Promise<void>;
    };
    deploy(data: string, args: any[]): Promise<any>;
    static deployed(web3: Web3, defaults: Tx): Promise<HistoricalPriceFeedContract>;
    static at(address: string, web3: Web3, defaults: Tx): Promise<HistoricalPriceFeedContract>;
    constructor(web3ContractInstance: Contract, defaults: Tx);
}
