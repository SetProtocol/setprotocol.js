...
contract('PreCoordinator', accounts => {
    ...
    describe('#createServiceAgreement', () => {
        context('when called by the owner', () => {
            it('emits the NewServiceAgreement log', async () => {
                const { logs } = await pc.createServiceAgreement(3, [oc1.address, oc2.address, oc3.address, oc4.address], [job1, job2, job3, job4], [payment, payment, payment, payment], { from: defaultAccount });
                openzeppelin_test_helpers_1.expectEvent.inLogs(logs, 'NewServiceAgreement');
            });
            it('creates a service agreement', async () => {
                const tx = await pc.createServiceAgreement(3, [oc1.address, oc2.address, oc3.address, oc4.address], [job1, job2, job3, job4], [payment, payment, payment, payment], { from: defaultAccount });
            ...
            it('does not allow service agreements with 0 minResponses', async () => {
                await openzeppelin_test_helpers_1.expectRevert(pc.createServiceAgreement(0, [oc1.address, oc2.address, oc3.address, oc4.address], [job1, job2, job3, job4], [payment, payment, payment, payment], { from: defaultAccount }), 'Min responses must be > 0');
            });
        });
        context('when called by a stranger', () => {
            it('reverts', async () => {
                await openzeppelin_test_helpers_1.expectRevert.unspecified(pc.createServiceAgreement(3, [oc1.address, oc2.address, oc3.address, oc4.address], [job1, job2, job3, job4], [payment, payment, payment, payment], { from: stranger }));
            });
        });
        context('when the array lengths are not equal', () => {
            it('reverts', async () => {
                await openzeppelin_test_helpers_1.expectRevert(pc.createServiceAgreement(3, [oc1.address, oc2.address, oc3.address, oc4.address], [job1, job2, job3], [payment, payment, payment, payment], { from: defaultAccount }), 'Unmet length');
            });
        });
        context('when the min responses is greater than the oracles', () => {
            it('reverts', async () => {
                await openzeppelin_test_helpers_1.expectRevert(pc.createServiceAgreement(5, [oc1.address, oc2.address, oc3.address, oc4.address], [job1, job2, job3, job4], [payment, payment, payment, payment], { from: defaultAccount }), 'Invalid min responses');
            });
        });
    });
    describe('#deleteServiceAgreement', () => {
        ...
        context('when the service agreement is still active', () => {
            ...
            it('reverts', async () => {
                await openzeppelin_test_helpers_1.expectRevert(pc.deleteServiceAgreement(saId, { from: defaultAccount }), 'Cannot delete while active');
            });
        });
    });
    describe('#onTokenTransfer', () => {
        context('when called by an address other than the LINK token', () => {
            it('reverts', async () => {
                ...
                const saId = await pc.createServiceAgreement.call(3, [oc1.address, oc2.address, oc3.address, oc4.address], [job1, job2, job3, job4], [payment, payment, payment, payment], { from: defaultAccount });
        ...
        context('when called by the LINK token', () => {
            ...
            it('creates Chainlink requests', async () => {
                ...
                assert.isTrue(new openzeppelin_test_helpers_1.BN(1).eq(serviceAgreement.activeRequests));
            });
            ...
            context('when the same nonce is used twice', () => {
                ...
                it('reverts', async () => {
                    await openzeppelin_test_helpers_1.expectRevert.unspecified(h.requestDataFrom(pc, link, totalPayment, args));
                });
            });
    ...
    describe('#chainlinkCallback', () => {
        ...
        context('when consumer is different than requester', () => {
            ...
            beforeEach(async () => {
                ...
                const reqTx = await rc.requestEthereumPriceByCallback(currency, totalPayment, cc.address, { from: consumer });
    ...
    describe('#cancelOracleRequest', () => {
        ...
        context('after the minimum required time', () => {
            ...
            it('does not allow others to call', async () => {
                await openzeppelin_test_helpers_1.expectRevert(pc.cancelOracleRequest(request.id, request.payment, request.callbackFunc, request.expiration, { from: stranger }), 'Only requester can cancel');
            });
        });
    });
});
