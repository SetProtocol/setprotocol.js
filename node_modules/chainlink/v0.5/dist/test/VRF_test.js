...
const fieldSize = helpers_1.bigNum('0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F');
const groupOrder = helpers_1.bigNum(
// Number of elements in the set {(x,y) in GF(fieldSize)^2 | y^2=x^3+3}
'0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141');
const generator = [
    '0x79BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798',
    '0x483ADA7726A3C4655DA4FBFC0E1108A8FD17B448A68554199C47D08FFB10D4B8',
].map(helpers_1.bigNum); // Point in EC group
...
const big1 = helpers_1.bigNum(1);
const big2 = helpers_1.bigNum(2);
const big3 = helpers_1.bigNum(3);
const assertPointsEqual = (x, y) => {
    matchers_1.assertBigNum(x[0], y[0]);
    matchers_1.assertBigNum(x[1], y[1]);
};
// Returns the EIP55-capitalized ethereum address for this secp256k1 public key
const toAddress = k => {
    return ethereumjs_util_1.pubToAddress(Buffer.concat(k.map(v => v.toBuffer()))).toString('hex');
};
contract('VRF', () => {
    let VRF;
    beforeEach(async () => {
    ...
    it('Accurately calculates simple and obvious bigModExp test inputs', async () => {
        ...
        matchers_1.assertBigNum(await VRF.bigModExp(3, 2, 5), rawExp % 5);
    });
    it('accurately calculates the sum of g and 2g (i.e., 3g)', async () => {
        const projectiveResult = await VRF.projectiveECAdd(generator[0], generator[1], twiceGenerator[0], twiceGenerator[1]);
    ...
    it('Can compute square roots', async () => {
        matchers_1.assertBigNum(2, await VRF.squareRoot(4), '4=2^2'); // 4**((fieldSize-1)/2)
    });
    it('Can compute the square of the y ordinate given the x ordinate', async () => {
        matchers_1.assertBigNum(8, await VRF.ySquared(1), '8=1^3+7');
    });
    it('Hashes to the curve with the same results as the golang code', async () => {
        let result = await VRF.hashToCurve(generator, 1);
        matchers_1.assertBigNum(helpers_1.bigNum(result[0])
            .pow(big3)
            .add(helpers_1.bigNum(7))
            .umod(fieldSize), helpers_1.bigNum(result[1])
            .pow(big2)
            .umod(fieldSize), 'y^2=x^3+7');
        // See golang code
        result = await VRF.hashToCurve(generator, 1);
        matchers_1.assertBigNum(result[0], '0x530fddd863609aa12030a07c5fdb323bb392a88343cea123b7f074883d2654c4', 'mismatch with output from services/vrf/vrf_test.go/TestVRF_HashToCurve');
        matchers_1.assertBigNum(result[1], '0x6fd4ee394bf2a3de542c0e5f3c86fc8f75b278a017701a59d69bdf5134dd6b70', 'mismatch with output from services/vrf/vrf_test.go/TestVRF_HashToCurve');
    });
    it('Correctly verifies linear combinations with generator', async () => {
        assert(await VRF.verifyLinearCombinationWithGenerator(5, twiceGenerator, 7, toAddress(seventeenTimesGenerator)), '5*(2*g)+7*g=17*g?');
    });
    it('Correctly computes full linear combinations', async () => {
        const projSum = await VRF.projectiveECAdd(eightTimesGenerator[0], eightTimesGenerator[1], nineTimesGenerator[0], nineTimesGenerator[1]);
        const zInv = projSum[2].invm(fieldSize);
        assertPointsEqual(seventeenTimesGenerator, await VRF.linearCombination(4, twiceGenerator, eightTimesGenerator, 3, thriceGenerator, nineTimesGenerator, zInv), '4*(2*g)+3*(3*g)=17*g?');
    });
    it('Computes the same hashed scalar from curve points as the golang code', async () => {
        const scalar = await VRF.scalarFromCurve(generator, generator, generator, toAddress(generator), generator);
        matchers_1.assertBigNum('0x2b1049accb1596a24517f96761b22600a690ee5c6b6cadae3fa522e7d95ba338', scalar, 'mismatch with output from services/vrf/vrf_test.go/TestVRF_ScalarFromCurve');
    });
    it('Knows a good VRF proof from bad', async () => {
        ...
        const bOutput = ethereumjs_util_1.keccak256(Buffer.concat(gamma.map(v => v.toBuffer())));
        const output = helpers_1.bigNum('0x' + bOutput.toString('hex'));
        assert(await checkOutput(output), 'rejected good proof');
    });
});
//# sourceMappingURL=VRF_test.js.map