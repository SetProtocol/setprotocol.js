...
exports.assertActionThrows = (action, messageContains) => Promise.resolve()
    ...
    .then(errorMessage => {
    assert(errorMessage && errorMessage.includes, 'Expected an error to be raised');
...
exports.parseAggregatorRoundLog = (log) => {
    const data = abi.rawDecode(['uint32', 'uint32'], util.toBuffer(log.data));
    return {
        paymentAmount: exports.bigNum(log.topics[1]),
        minAnswerCount: exports.bigNum(log.topics[2]),
        maxAnswerCount: exports.bigNum(log.topics[3]),
        restartDelay: data[0],
        timeout: data[1],
    };
};
