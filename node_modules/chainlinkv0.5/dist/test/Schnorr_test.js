...
const dssTest_1 = __importDefault(require("./dssTest"));
...
contract('SchnorrSECP256K1', async () => {
    ...
    const e = hexToBN(web3.utils.soliditySha3(toHex(publicKey[0], 256), toHex(pubKeyYParity ? '0x01' : '0x00', 8), toHex(msgHash, 256), toHex(kTimesGAddress, 160)));
    ...
    it('Knows a good Schnorr signature from bad', async () => {
        assert(publicKey[0].lt(groupOrder.shrn(1).add(bigOne)), 'x ordinate of public key must be less than half group order.');
        const checkSignature = async (s) => c.verifySignature.call(publicKey[0], pubKeyYParity, s, msgHash, kTimesGAddress);
        assert(await checkSignature(s), 'failed to verify good signature');
        assert(!(await checkSignature(s.add(bigOne))), // Corrupt signature for
        'failed to reject bad signature');
        const gasUsed = await c.verifySignature.estimateGas(publicKey[0], pubKeyYParity, s, msgHash, kTimesGAddress);
        assert.isBelow(gasUsed, 37500, 'burns too much gas');
    });
    it('Accepts the signatures generated on the go side', async () => {
        tests_1.default.push(dssTest_1.default);
        for (let i = 0; i < Math.min(1, tests_1.default.length); i++) {
            const numbers = tests_1.default[i].slice(0, tests_1.default[i].length - 1);
            const [msgHash, secret, pX, pY, sig] = numbers.map(hexToBN);
            const rEIP55Address = web3.utils.toChecksumAddress(tests_1.default[i].pop());
            secret.and(bigOne); // shut linter up about unused variable
            assert(await c.verifySignature.call(pX, pY.isEven() ? 0 : 1, sig, msgHash, rEIP55Address), 'failed to verify signature constructed by golang tests');
            assert(!(await c.verifySignature.call(pX, pY.isEven() ? 0 : 1, sig.add(bigOne), msgHash, rEIP55Address)), 'failed to reject bad signature');
        }
    });
});
//# sourceMappingURL=Schnorr_test.js.map