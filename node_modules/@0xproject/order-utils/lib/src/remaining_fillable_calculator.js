...
var RemainingFillableCalculator = /** @class */ (function () {
    function RemainingFillableCalculator(orderFee, orderAssetAmount, isTraderAssetZRX, transferrableAssetAmount, transferrableFeeAmount, remainingOrderAssetAmount) {
    ...
    RemainingFillableCalculator.prototype._hasSufficientFundsForFeeAndTransferAmount = function () {
        if (this._isTraderAssetZRX) {
            var totalZRXTransferAmountRequired = this._remainingOrderAssetAmount.plus(this._remainingOrderFeeAmount);
            var hasSufficientFunds = this._transferrableAssetAmount.greaterThanOrEqualTo(totalZRXTransferAmountRequired);
            return hasSufficientFunds;
        }
        else {
            var hasSufficientFundsForTransferAmount = this._transferrableAssetAmount.greaterThanOrEqualTo(this._remainingOrderAssetAmount);
            var hasSufficientFundsForFeeAmount = this._transferrableFeeAmount.greaterThanOrEqualTo(this._remainingOrderFeeAmount);
    ...
    RemainingFillableCalculator.prototype._calculatePartiallyFillableAssetAmount = function () {
        ...
        if (this._isTraderAssetZRX) {
            ...
            var totalZRXTokenPooled = this._transferrableAssetAmount;
            // The purchasing power here is less as the tokens are taken from the same Pool
            // For every one number of fills, we have to take an extra ZRX out of the pool
            fillableTimesInAssetUnits = totalZRXTokenPooled.dividedBy(orderToFeeRatio.plus(new utils_1.BigNumber(1)));
        }
